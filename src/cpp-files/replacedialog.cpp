...
replaceDialog::replaceDialog( QWidget* parent, QTextEdit* textFile,
                              QTextCursor cursor )
...
void replaceDialog::on_pushReplace_clicked() {
    ...
    // take the value of fileText and convert it to wide string and assign to
    // Text
    ...
    int found =
        Text.find( findWord.toStdWString(), location - findWord.length() );
    ...
    if ( found != -1 ) {
        ...
        Text.replace( location - findWord.length(), findWord.length(),
                      ui->txtReplace->text().toStdWString() );
...
void replaceDialog::on_pushRAll_clicked() {
    ...
    if ( found != -1 ) {
        ...
        QMessageBox::information(
            this, "Replace All",
            "All <b>" + findWord + "</b> words are replaced with <b>" +
