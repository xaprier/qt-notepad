...
void findDialog::pushFind() {
    // if findWord equal to empty string(not coming from replaceDialog class) findWord value will be our txtFind box's value
    if (findWord == "") {
        findWord = findui->txtFind->text();
    } else {
        // else(coming from replaceDialog class) assign the values of replaceDialog class's location and count value
    ...
    // if function caller is replaceDialog class, assign the findDialog class's location and count variables to replaceDialog's variables.
