  ...
  if (!inText.isEmpty() && !(QWidget::windowTitle().endsWith("~"))) {
    ...
    if (inText != inText2) {
      ...
      reply = QMessageBox::question(
          this, "Save File", "File not saved. Save changes before closing?",
...
void mainwindow::selectFont() {
  ...
  ui->textEdit->setFontFamily(newFont.family()), setFontBold(newFont.bold()),
      setFontItalic(newFont.italic()), setFontUnderLine(newFont.underline());
...
void mainwindow::about() {
  QMessageBox::about(
      ...
      tr("The <b>Notepad</b> example demonstrates how to code a basic "
         "text editor using Qt::Widgets. <br><b>Coded by xaprier</b>"));
}
