...
findDialog::findDialog(QWidget *parent, QTextEdit *textFile, QTextCursor cursor,
                       QString fW)
...
void findDialog::pushFind() {
  ...
  if (location == 0) {
    for (size_t i = Text.find(findWord.toStdWString()); i != std::string::npos;
         i = Text.find(findWord.toStdWString(), i + findWord.length())) {
      count++;
    }
  }
